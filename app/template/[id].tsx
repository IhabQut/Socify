...
import Animated, { FadeIn, FadeInDown, FadeInUp, Layout, useAnimatedStyle, useSharedValue, withSpring, SlideInUp, SlideOutDown } from 'react-native-reanimated';
...
// Updated Mock DB with premium thumbnails
const MOCK_TEMPLATES: Record<string, DesignTemplate> = {
    '1': {
        id: '1', title: 'Black Friday Frenzy', category: 'Instagram Posts',
        thumbnail_url: 'https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?q=80&w=1000',
        video_preview_url: 'https://assets.mixkit.co/videos/preview/mixkit-shopping-bags-in-a-mall-4881-large.mp4',
        tutorial_video_url: 'https://assets.mixkit.co/videos/preview/mixkit-software-developer-working-on-his-computer-34440-large.mp4',
        requirements: [
    ...
    '4': {
        ...
        tutorial_video_url: 'https://assets.mixkit.co/videos/preview/mixkit-presentation-of-a-new-modern-house-41311-large.mp4',
        requirements: [
    ...
    '6': {
        ...
        thumbnail_url: 'https://images.unsplash.com/photo-1492691527719-9d1e07e534b4?q=80&w=1000',
        tutorial_video_url: 'https://assets.mixkit.co/videos/preview/mixkit-traveler-woman-on-a-viewpoint-in-the-mountains-43407-large.mp4',
        requirements: [
    ...
    return (
        <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
            <View style={[styles.tutorialOverlay, { backgroundColor: '#000' }]}>
                <View style={[styles.tutorialHeader, { paddingTop: 50 }]}>
                    <Pressable style={styles.tutorialClose} onPress={onClose}>
...
const RequirementInput = ({ req, theme, isLast, value, onValueChange }: any) => {
    ...
    return (
        <Animated.View layout={Layout.springify()} style={[styles.reqBlock, !isLast && { borderBottomWidth: 1.5, borderBottomColor: theme.card }]}>
            <PickerSheet
                visible={showSheet}
            ...
            <View style={styles.reqHeader}>
                <View style={[styles.reqIcon, { backgroundColor: theme.background }]}>
                    {req.type === 'photo' && <Ionicons name="image" size={18} color={theme.primary} />}
                    {req.type === 'video' && <Ionicons name="videocam" size={18} color={theme.primary} />}
                    {req.type === 'text' && <Ionicons name="text" size={18} color={theme.primary} />}
                </View>
                <View style={styles.reqInfo}>
                    <Text style={[styles.reqLabel, { color: theme.text }]}>{req.label}</Text>
                    {req.description && <Text style={[styles.reqDesc, { color: theme.icon }]} numberOfLines={1}>{req.description}</Text>}
                </View>
                {pickedUri && (
                    <Pressable onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); onValueChange(null); }} hitSlop={15}>
            ...
            {(req.type === 'photo' || req.type === 'video') ? (
                pickedUri ? (
                    <Animated.View entering={FadeIn.duration(400)} style={styles.previewContainer}>
                        <Pressable onPress={handleUploadPress} style={{ flex: 1 }}>
                    ...
                    <AnimatedPressable
                        style={[styles.uploadBox, animatedStyle, { backgroundColor: theme.card + '40', borderColor: theme.border }]}
                        onPressIn={() => { scale.value = withSpring(0.98); }}
                        onPressOut={() => scale.value = withSpring(1)}
                        onPress={handleUploadPress}
                    >
                        <Ionicons name="add" size={32} color={theme.primary} opacity={0.5} />
                        <Text style={[styles.uploadText, { color: theme.text }]}>Add {req.type}</Text>
                    </AnimatedPressable>
                )
            ) : (
                <TextInput
                    style={[styles.textInput, { backgroundColor: theme.card + '40', color: theme.text, borderColor: theme.border }]}
    ...
    useEffect(() => {
        async function fetchTemplate() {
            ...
            if (data) {
                ...
                // Force fallback for tutorial URL for ALL templates to ensure button is visible
                const mockTutorial = MOCK_TEMPLATES[id as string]?.tutorial_video_url;
                setTemplate({
                    ...data,
                    tutorial_video_url: data.tutorial_video_url || mockTutorial || 'https://assets.mixkit.co/videos/preview/mixkit-software-developer-working-on-his-computer-34440-large.mp4'
                });
            ...
            <View style={[styles.container, { backgroundColor: theme.background }]}>
                ...
                <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} bounces={false}>
                    ...
                    <View style={[styles.heroSection, { backgroundColor: theme.card }]}>
                        <Image 
                            source={{ uri: template.thumbnail_url || 'https://images.unsplash.com/photo-1614850523296-d8c1af93d400?q=80&w=1000' }} 
                            style={styles.heroBackground} 
                        />
                        <BlurView intensity={colorScheme === 'dark' ? 60 : 80} tint={colorScheme} style={styles.heroOverlay}>
                            <View style={[styles.heroContent, { paddingTop: insets.top + 10 }]}>
                                <View style={styles.heroTopActions}>
                                    <Pressable 
                                        style={[styles.circleAction, { backgroundColor: theme.card }]} 
                                    ...
                                    <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center' }}>
                                        {template.tutorial_video_url && (
                                            <Pressable 
                                                style={[styles.tutorialBtn, { backgroundColor: theme.accent }]} 
                                                onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); setShowTutorial(true); }}
                                            >
                                ...
                                <View style={styles.heroBottomInfo}>
                                    <Text style={[styles.heroCategoryLabel, { color: theme.primary }]}>{template.category || 'Creative Studio'}</Text>
                                    <Text style={[styles.heroMainTitle, { color: theme.text }]}>{template.title}</Text>
                                    <View style={styles.statsRow}>
                                        <View style={styles.statItem}>
                                            <Ionicons name="time-outline" size={14} color={theme.icon} />
                                            <Text style={[styles.statText, { color: theme.icon }]}>Fast Gen</Text>
                                        </View>
                                        <View style={styles.statDot} />
                                        <View style={styles.statItem}>
                                            <Ionicons name="layers-outline" size={14} color={theme.icon} />
                                            <Text style={[styles.statText, { color: theme.icon }]}>{template.requirements?.length || 0} Assets</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                    ...
                    {template.tutorial_video_url && (
                        <TutorialVideoModal 
                            ...
                            videoUrl={template.tutorial_video_url} 
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={[styles.cardContainer, { backgroundColor: theme.card, borderColor: theme.border }]}>
                            ...
                            <View style={[styles.prefRow, { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
                                ...
                                <Pressable
                                    style={[styles.toggle, { backgroundColor: aiCaption ? theme.accent : theme.background, borderColor: aiCaption ? theme.accent : theme.border }]}
                                    onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setAiCaption(!aiCaption); }}
                                >
                                    <Animated.View layout={Layout.springify()} style={[styles.knob, { left: aiCaption ? 22 : 2, backgroundColor: '#FFF' }]} />
                                </Pressable>
                            </View>
...
const styles = StyleSheet.create({
    ...
    heroSection: { height: height * 0.4, overflow: 'hidden' },
    heroBackground: { ...StyleSheet.absoluteFillObject },
    heroOverlay: { ...StyleSheet.absoluteFillObject },
    heroContent: { flex: 1, padding: 24, justifyContent: 'space-between' },
    heroTopActions: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    circleAction: { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center' },
    tutorialBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20 },
    tutorialBtnText: { color: '#FFF', fontSize: 13, fontWeight: '700' },
    ...
    heroBottomInfo: { gap: 4 },
    heroCategoryLabel: { fontSize: 12, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 1 },
    heroMainTitle: { fontSize: 32, fontWeight: '900', letterSpacing: -1 },
    statsRow: { flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 4 },
    statItem: { flexDirection: 'row', alignItems: 'center', gap: 4 },
    statText: { fontSize: 13, fontWeight: '600' },
    statDot: { width: 4, height: 4, borderRadius: 2, backgroundColor: 'rgba(255,255,255,0.3)' },

    section: { paddingHorizontal: 20, marginTop: 24 },
    sectionTitle: { fontSize: 20, fontWeight: '800', marginBottom: 16, letterSpacing: -0.5 },
    cardContainer: { borderRadius: 28, borderWidth: 1, overflow: 'hidden' },

    reqBlock: { padding: 20 },
    reqHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
    reqIcon: { width: 44, height: 44, borderRadius: 14, justifyContent: 'center', alignItems: 'center', marginRight: 16 },
    reqInfo: { flex: 1 },
    reqLabel: { fontSize: 16, fontWeight: '700', marginBottom: 2 },
    reqDesc: { fontSize: 13, fontWeight: '500' },
    
    uploadBox: { height: 100, borderRadius: 20, borderWidth: 1.5, borderStyle: 'dotted', justifyContent: 'center', alignItems: 'center', gap: 4 },
    uploadText: { fontSize: 14, fontWeight: '700' },
    textInput: { paddingHorizontal: 16, paddingVertical: 14, borderRadius: 16, borderWidth: 1, fontSize: 15, fontWeight: '600' },

    previewContainer: { borderRadius: 20, overflow: 'hidden', height: 200, backgroundColor: 'rgba(0,0,0,0.05)' },
    previewImage: { width: '100%', height: '100%' },
    previewBadge: { position: 'absolute', top: 12, right: 12, width: 28, height: 28, borderRadius: 14, justifyContent: 'center', alignItems: 'center' },
    ...
    tutorialOverlay: { flex: 1, backgroundColor: '#000' },
    tutorialHeader: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingBottom: 20 },
    tutorialClose: { marginRight: 16 },
    tutorialTitle: { color: '#FFF', fontSize: 18, fontWeight: '800' },
    tutorialPlayer: { flex: 1, width: '100%' },
});
